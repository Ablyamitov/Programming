...
class Multiplayer():
    ...
    def search_player_button(self):
        ...
        #self.screen.fill(self.button_color,self.search_player_rect)
        ...
        inst3 = 'Победа в игре - у противника 0 хп'
    ...
    def blit_enemy_elem(self,my_elem,rez):
        ...
        enemy_rock_rect.x =  enemy_rock_rect.x + enemy_rock_rect.width

        enemy_paper_rect = copy.deepcopy(self.paper_rect)
        enemy_paper_rect.x = enemy_paper_rect.x + enemy_paper_rect.width

        enemy_scissors_rect = copy.deepcopy(self.scissors_rect)
        enemy_scissors_rect.x = enemy_scissors_rect.x  +enemy_scissors_rect.width
    ...
    def blit_end_multiplayer_game(self,rez):
        if rez == 'WIN':
            end_game_text = 'Подзравляю, ты победил!'
        elif rez == 'LOSE':
            end_game_text = 'К сожалению, ты проиграл'
        end_game_text_image = self.font.render(end_game_text,True,self.text_color)
        ...
        exit_text_image = self.font.render(exit_text,True,self.text_color)
        exit_text_rect = exit_text_image.get_rect()
        exit_text_rect.center = self.screen_rect.center
        #exit_text_rect.x = exit_text_rect.x-(exit_text_rect.width/2)
