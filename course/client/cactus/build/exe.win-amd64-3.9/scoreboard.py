...
class Scoreboard():
    ...
    def __init__(self,main):
        ...
        self.instruction = 'Инструкция к игре:'
        self.instruction2 = 'Ваша задача: успеть нажать на соответствующую стрелочке клавишу в области квадрата'
        self.instruction3 = 'При попадании +50 очков, при промахе -10 очков.'
        self.instruction4 = 'При достижении 250 очков кактус поливается, +1 монета и +100 exp, а очки сбрасываются.'
        self.instruction5 = 'Главная цель: как можно больше полить кактус.'
        self.instruction6 = 'Начать игру - клавиша SPACE или нажатие кнопки "Play", выход - клавиша ESC.'
        self.countdown = 3
    ...
    def end_game(self,coins,exp):
        ...
        points_rect.centerx = self.screen_rect.centerx
        ...
        watering_rect.centerx = self.screen_rect.centerx
        ...
        back_to_menu_rect.midbottom = self.screen_rect.midbottom
        self.screen.blit(back_to_menu_image,back_to_menu_rect)

        coins_txt = 'Заработано монет: '+str(coins)
        coins_image = self.font.render(coins_txt,True,self.stats.text_color)
        coins_rect = coins_image.get_rect()
        coins_rect.centerx = self.screen_rect.centerx
        coins_rect.y = watering_rect.y + self.end_rect.height + 50
        self.screen.blit(coins_image,coins_rect)

        exp_txt = 'Заработано опыта: '+str(exp)
        exp_image = self.font.render(exp_txt,True,self.stats.text_color)
        exp_rect = exp_image.get_rect()
        exp_rect.centerx = self.screen_rect.centerx
        exp_rect.y = coins_rect.y + self.end_rect.height + 50
